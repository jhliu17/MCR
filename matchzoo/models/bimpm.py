...
class BiMPM(nn.Module):
    def __init__(self, config, pipeline: RHPPipeline, stage: str):
        ...
        self.context_LSTM = nn.LSTM(
            input_size=config.prd_txt_encoder.embedding.embed_dim,
            hidden_size=config.prd_txt_encoder.encoder.hidden_dimension,
            num_layers=1,
            bidirectional=True,
            batch_first=True
        )
        ...
        for i in range(1, 9):
            setattr(self, f'mp_w{i}',
                    nn.Parameter(torch.rand(config.multi_perceptive.num_perspective,
                                            config.prd_txt_encoder.encoder.hidden_dimension)))

        # Aggregation Layer
        self.aggregation_LSTM = nn.LSTM(
            input_size=config.multi_perceptive.num_perspective * 8,
            hidden_size=config.prd_txt_encoder.encoder.hidden_dimension,
            num_layers=1,
            bidirectional=True,
            batch_first=True
        ...
        self.pred_fc1 = nn.Linear(
            config.prd_txt_encoder.encoder.hidden_dimension * 4,
            config.prd_txt_encoder.encoder.hidden_dimension * 2)
        self.pred_fc2 = nn.Linear(
            config.prd_txt_encoder.encoder.hidden_dimension * 2, 1)
    ...
    def forward(self, inputs):
        ...
        # [B, L, D]
        # [B, R, D]
        p = self.q_embedding(p)
        h = self.d_embedding(h)
        ...
        con_p_fw, con_p_bw = torch.split(con_p,
                                         self.config.prd_txt_encoder.encoder.hidden_dimension,
                                         dim=-1)
        ...
        x = torch.cat(
            [agg_p_last.permute(1, 0, 2).contiguous().view(
                -1, self.config.prd_txt_encoder.encoder.hidden_dimension * 2),
    ...
    def dropout(self, v):
        """Dropout Layer."""
        return F.dropout(v, p=self.config.input_setting.dropout, training=self.training)
